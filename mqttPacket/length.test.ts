...
Deno.test('decodeLength', function decodeLength() {
  ...
  assertThrows(()=>decode(Uint8Array.from([0xff, 0xff, 0xff, 0xff]),0), Error,"length decoding failed");
  assertThrows(()=>decode(Uint8Array.from([0xff, 0xff, 0xff, 0xff, 0x7f]),0), Error,"length decoding failed");
  assertThrows(()=>decode(Uint8Array.from([0xff]),0), Error,"length decoding failed");
  assertThrows(()=>decode(Uint8Array.from([]),0), Error,"length decoding failed");
});